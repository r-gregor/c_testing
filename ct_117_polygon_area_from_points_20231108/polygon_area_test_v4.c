...
/* === USING FUNCTIONS ====================== */
	...
	Polygon p2 = make_polygon(square_points, sizeof(square_points) / sizeof(square_points[0])); // v4
...
int polygon_area(Polygon *p) {
	 ...
	 * area is on the left)
	 * ---
	 *  v3 -- no need for points_num - 1
	 */
	for (int i=0; i < p->points_num; i++) {
		dsum += (p->points_arr[i-1].x) * (p->points_arr[i].y) - (p->points_arr[i-1].y) * (p->points_arr[i].x);
	}
