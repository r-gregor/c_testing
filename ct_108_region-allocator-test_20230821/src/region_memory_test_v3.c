...
int main(int argc, char **argv) {
	...
	// allocate memory
	char *story = region_alloc(&r, 150*1024*1024);            // num of chars between "" + at least 1
	
	// read from file into memory region
	fread(story, 150, 1024*1024, fp);
	fclose(fp);
	...
	fwrite( story + curr_point, 1024, n, stdout); // as long as (story + curr_point + 1024 * n) <= 150*1024*1024
	printf("\n");
