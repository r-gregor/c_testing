 ...
 *  ---
 *  v8: continue from : 49:56 / 1:51:27
 *      convert absolute to relative pointers
 *      STOPPED AT: 1:01:33 / 1:51:27 (20230829 - EN)
 */
#include <stdio.h>
#include <string.h>
#include <assert.h>
...
// macro (**1)
// aptr - absolute pointer (the value of memory address)
...
struct Node {
	...
	// NEW:
	int32_t left;    // relative pointer (2x32 -> 64), signed so it can point to forward or backward
	int32_t right;   // relative pointer (2x32 -> 64), signed so it can point to forward or backward
};
...
/* v8 convert absolute pointer to relative */
Node *random_tree(Node_Pool *np, size_t level) {
	...
	// v8:
	// root->left  = (int32_t)((uint8_t *)random_tree(np, level -1) - (uint8_t *)&root->left); // recursion
	// root->right  = (int32_t)((uint8_t *)random_tree(np, level -1) - (uint8_t *)&root->right); // recursion
	// after macro (**1):
	abs2rel32(root->left, random_tree(np, level -1));
	abs2rel32(root->right, random_tree(np, level -1));
