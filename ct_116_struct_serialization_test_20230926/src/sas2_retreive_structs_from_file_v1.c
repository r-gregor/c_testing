/*
 * sas2_retreive_structs_from_file_v1.c
 *
 * 20230927_02 (en)
 */
#include <stdio.h>
...
/*
 * Have to know definition of struct to decompose buf into
 * individual struct
 */
...
void print_single_structs(char *buf, uint64_t count, uint64_t structs_count);
...
int main(void) {
	...
	print_single_structs((char *)&buffer, 8, structs_count); // test: outside range
	print_single_structs((char *)&buffer, 3, structs_count);
	print_single_structs((char *)&buffer, 6, structs_count);
...
void print_all_structs(char *buf, uint64_t structs_count) {
	for (uint64_t count = 1; count <= structs_count; count++) {
		print_single_structs(buf, count, structs_count);
	}
}
