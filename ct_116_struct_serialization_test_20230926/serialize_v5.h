...
void element_add_to_buf(char *buf, char *name) {
	uint64_t element_offset = sizeof(Element_t) * elements_global_count;
	uint64_t element_num = elements_global_count + 1;

	if (element_offset >= BUF_SIZE) {
		printf("Buffer full!\n");
		return;
	}
	...
	Element_t *new_element = (Element_t *)(buf + element_offset);
	new_element->offset = element_offset; // v4
