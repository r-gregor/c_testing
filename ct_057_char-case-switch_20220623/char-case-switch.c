...
/*
 * v2
 * changed check for case from external islower() function to
 * char number check
 */
...
int main(int argc, char **argv) {
	...
	char *lowers = "abcdefghijkl # mnopqrs ~ tuvwxyz";
	char *lowers2 = "abCdefGhijk # lMnopQr ~ stuvWxyZ";
...
char lower_to_upper(char lower_char) {
	if (lower_char >= 'a' && lower_char <= 'z') { // v2
		return lower_char + ('A' - 'a');
	} else {
		return lower_char;
	}
