#include <stdio.h>
#include <string.h>
#include <stdlib.h>
#include "hashtable.h"

typedef struct entry {
	char *key;
	/* size_t keylength;		// refactoring v1 */
	/*							// must remove 'size_t keylen' from hahtable.h */
	void *object;
	struct entry *next;

} entry;

typedef struct _hash_table {
	uint32_t size;
	hashfunction *hash;
	entry **elements;
} hash_table;

static size_t hash_table_index(hash_table *ht, const char *key) {
	size_t result = (ht->hash(key, strlen(key)) % ht->size); // produces uint64_t so mod to size
	return result;
}

hash_table *hash_table_create(const uint32_t size, hashfunction *hf) {
	hash_table *ht = malloc(sizeof(*ht));
	ht->size = size;
	ht->hash = hf;
	ht->elements = calloc(sizeof(entry *), ht->size); // calloc zeroes out memory!
	return ht;
}

void hash_table_destroy(hash_table *ht) {
	// TODO: individual elements
	
	free(ht->elements);
	free(ht);
}

void hash_table_print(hash_table *ht) {
	printf("Start Table\n");
	getchar();
	for (uint32_t i=0; i < ht->size; i++) {
		if (ht->elements == NULL) {
			// print null?
			// printf("\t%i\t---\n", i);
		} else {
			printf("\t%i\t\n", i);
			entry *tmp = ht->elements[i];

			while (tmp != NULL) {
				printf("\"%s\"(%p) - ", tmp->key, tmp->object);
				tmp = tmp->next;
			}
			printf("\n");
		}
	}
	printf("End Table\n");
}

// refactoring v1
// bool hash_table_insert(hash_table *ht, const char *key, size_t keylen, void *obj) {
bool hash_table_insert(hash_table *ht, const char *key, void *obj) {
	if(key == NULL || obj == NULL || ht == NULL) return false;
	size_t index = hash_table_index(ht, key);

	if (hash_table_lookup(ht, key) != NULL) return false;

	// create a new entry
	entry *e = malloc(sizeof(*e));
	e->object = obj;
	e->key = malloc(strlen(key) + 1);
	strcpy(e->key, key);

	// insert entry
	e->next = ht->elements[index];
	ht->elements[index] = e;

	return true;
}

// refactoring v1
// void *hash_table_lookup(hash_table *ht, const char *key, size_t keylen) {
void *hash_table_lookup(hash_table *ht, const char *key) {
	if(key == NULL || ht == NULL) return false;
	size_t index = hash_table_index(ht, key);

	entry *tmp = ht->elements[index];
	while (tmp != NULL && strcmp(tmp->key, key) != 0) {
		tmp = tmp->next;
	}
	// failed: got to the end, and no find
	if (tmp == NULL) return NULL;

	//success:
	return tmp->object;
}

// refactoring v1
// void *hash_table_delete(hash_table *ht, const char *key, size_t keylen) {
void *hash_table_delete(hash_table *ht, const char *key) {
	if(key == NULL || ht == NULL) return false;
	size_t index = hash_table_index(ht, key);

	entry *tmp = ht->elements[index];

	entry *prev = NULL;
	while (tmp != NULL && strcmp(tmp->key, key) != 0) {
		prev = tmp;
		tmp = tmp->next;
	}
	// failed: got to the end, and no find
	if (tmp == NULL) return NULL;
	if (prev == NULL) {
		// deleting the head of linked list
		ht->elements[index] = tmp->next;
	} else {
		// deleting from somewhere not the head
		prev->next = tmp->next;
	}

	//success:
	void *result = tmp->object;
	free(tmp);
	return result;
}


