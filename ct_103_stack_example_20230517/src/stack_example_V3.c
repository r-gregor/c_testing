...
void test_if_empty(Stack *stack);
void test_if_full(Stack *stack);
...
#if 1
	// test 1
	printf("*** Adding elements to the stack ***\n");
	stack = create_stack(STACK_MAX_ELEMENTS);
	if (stack == NULL) {
		printf("[ERROR]: Could not create stack\n");
		return 1;
	} else {
		printf("Stack of size %d elements successfuly created, ", STACK_MAX_ELEMENTS);
		test_if_empty(stack);
	}
